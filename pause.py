...
def show_victory_screen(screen_surface, score: int = 0, coins: int = 0):
		"""Block until user clicks Quit to Main Menu. Returns when user chooses to quit."""
		sw, sh = screen_surface.get_size()
		clock = pygame.time.Clock()
		# slightly smaller fonts so the title fits comfortably in the panel
		...
		# stop any background music and play victory SFX once
		try:
			pygame.mixer.music.stop()
		except Exception:
			pass
		try:
			sounds.preload('VictorySound')  # expects sounds/VictorySound.(ogg|mp3|wav)
			sounds.play_sfx('VictorySound')
		except Exception:
			pass
		...
		while True:
				...
				if ev.type == pygame.KEYDOWN:
					if ev.key == pygame.K_ESCAPE:
						try:
							...
							pass
						# treat ESC as quit to menu on victory screen
						return ("menu", None)
				if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1:
					if btn_rect.collidepoint(ev.pos):
						try:
			...
			panel = pygame.Rect((sw-panel_w)//2, (sh-panel_h)//2, panel_w, panel_h)
			pygame.draw.rect(screen_surface, (30,30,30), panel)
			pygame.draw.rect(screen_surface, (20,150,20), panel, 3)

			title = title_f.render("You Win!", True, (220, 220, 180))
			screen_surface.blit(title, title.get_rect(center=(sw//2, panel.top + 56)))
